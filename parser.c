...
/* fonction permettant la création d'une nouvelle variable ayant pour numéro num */
{
  ...
  if(variables == NULL) variables=malloc(sizeof(variable));
  else variables = realloc(variables,sizeof(variable)*(num+1));
  variables[num].val=-1;
	if(sprintf(variables[num].name,"Var%d",num)<0){
	printf("Error sprintf in function Variable\n");
	exit(-1);
	}
  /*if(strcpy(variables[num].name,"var")== NULL){
	printf("Error strcpy in fonction Variable");
	exit(-1);
	}
  if(strcat(variables[num].name,numInChar)==NULL){
	printf("Error strcat in fonction Variable");
	exit(-1);
	}*/
  printf ("Variable %s\n",variables[num].name);
